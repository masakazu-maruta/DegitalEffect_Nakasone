...
export default class LinearOpeningExpandAnimator implements IOpeningExpandAnimator {
  public expand = async (htmlElement: HTMLElement, duration: number): Promise<void> => {
    ...
    return new Promise<void>((resolve) => {
      const x = Math.random() * GlobalViewportManager.getViewPortWidth() * 1.5 - GlobalViewportManager.getViewPortWidth() * 0.25;
      const y = Math.random() * GlobalViewportManager.getViewPortHeight() * 1.5 - GlobalViewportManager.getViewPortHeight() * 0.25;
