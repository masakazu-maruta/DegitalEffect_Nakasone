...
export default class ReverseFitFrameAnim implements IFrameAnim {
  ...
  public optimizeLeftFrame = () => {
    this.manager.leftFramePath.setAttribute("d", `M ${this.ratio2ViewPortWidth(this.offsetX)} ${this.getPathHeight()} V ${this.ratio2ViewPortHeight(1 - this.offsetX)} H ${this.getPathWidth()}`);
  ...
  public optimizeRightFrame = () => {
    this.manager.rightFramePath.setAttribute("d", `M ${this.ratio2ViewPortWidth(1 - this.offsetX)} ${this.ratio2ViewPortHeight(1)} V ${this.ratio2ViewPortHeight(this.offsetY)} H ${this.ratio2ViewPortWidth(1 - this.offsetY) - this.getPathWidth()}`);
    ...
    document.addEventListener("resize", () => {
      this.manager.rightFramePath.setAttribute("d", `M ${this.ratio2ViewPortWidth(1 - this.offsetX)} 0 V ${this.ratio2ViewPortHeight(1 - this.offsetY)} H ${this.ratio2ViewPortWidth(1 - this.offsetY) - this.getPathWidth()}`);
      this.manager.rightFramePath.setAttribute("stroke-dasharray", `${this.getPathLineLength()},${this.ratio2ViewPortHeight(1 - this.offsetY) + this.getPathWidth()}`);
      this.manager.rightFramePath.setAttribute("stroke-dashoffset", `${this.getPathLineLength()}`);
    });
  };
  ...
  public startLastTextAnim = async (duration: number) => {
    ...
    await new Promise<void>((resolve) =>
      setTimeout(() => {
        ...
        let hasRunLeftFrame = false;
        const triggerLength = this.getPathHeight();
        gsap.fromTo(
          ...
          },
          {
            ...
            ease: "none",
            onUpdate: () => {
              const currentStartOffset = parseInt(this.manager.lastTitleTextPath.getAttribute("startOffset") || "0");
              if (currentStartOffset > triggerLength && !hasRunLeftFrame) {
                hasRunLeftFrame = true;
                const pathLength = this.ratio2ViewPortHeight(1 - this.offsetY - 0.02) + textWidth;
                const speed = pathLength / duration;
                const leftFrameDuration = this.getPathLineLength() / speed;
                const leftAnim = this.startLeftFrameAnim(leftFrameDuration);
                promises.push(leftAnim);
              }
            },
            onComplete: () => {
              resolve();
