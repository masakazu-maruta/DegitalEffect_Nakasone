...
export default class RandomFactoryAnim implements IFactoryAnim {
  ...
  private parent: HTMLElement = document.getElementById("script-conveyerFlow") as HTMLElement;
  private finishedFlag: boolean = false;
  public factory: (duration: number) => void = async (duration: number) => {
  ...
  private randomGenerate = async (imageSrcs: string[], duration: number) => {
    ...
    const clothes: Clothes = new Clothes(this.parent, imageSrcs[randomSrcIndex]);
  ...
  private enrollAnimation = (clothes: Clothes, duration: number) => {
    ...
    gsap.fromTo(
      ...
      {
        bottom: `${parentHeight + 200}px`, // 終了位置
        duration: duration,
        ease: "linear",
        onUpdate: () => {
