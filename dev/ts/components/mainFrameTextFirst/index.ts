export class MainFrameTextFirst extends HTMLElement {
  ...
  setSize() {
    ...
    this.path.setAttribute("d", `M 0 ${-(halfLength - 10)} V ${height - 5}`);
    this.svgAnimate.setAttribute("values", `0;${halfLength + height - textLength - 100}`);
    this.text.setAttribute("transform", `translate(${this.text.getBBox().width / 2}, 0)`);
  }
}
